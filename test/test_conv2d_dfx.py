...
def test_conv2d_dfx():
    ...
    for i in range(iterations):
        ...
        # N H W C
        # activations = tf.get_variable("a", [t_n, t_h, t_w, t_ci], dtype=tf.float32, initializer=tf.glorot_normal_initializer())
        # # K K Ci Co
        # c2d_wts =     tf.get_variable("b", [w_k, w_k, t_ci, t_co], dtype=tf.float32, initializer=tf.glorot_normal_initializer())
        ...
        with tf.Session('') as sess:
            ...
            z_op = waveflow.wavecomp_ops_module.wave_conv2d_dfx(activations, c2d_wts, strides=[1, 1, 1, 1], padding='VALID')

            # Base tensorflow. Only supports NHWC.
            z2_op = tf.nn.conv2d(activations, c2d_wts,
                strides=[1, 1, 1, 1], padding='VALID', data_format='NHWC', use_cudnn_on_gpu=False)
            z, z2, act_val, wts_val = sess.run([z_op, z2_op, activations, c2d_wts])

            assert_str = "Failure on i: %d, mode: VALID, params: %s" % (i, str(p))
            if not compare_tensor(z, z2, assert_str):
                print("activations: %s" % (act_val))
                print("c2d_wts: %s" % (wts_val))
                print("\n\n")
