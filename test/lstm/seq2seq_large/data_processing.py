...
class DataCreator(object):
    ''' This object abstracts away reading in the dataset, both the input and the
        target data.
    '''
    def __init__(self, data_path, num_samples, max_sample_len):
        self._data_path = data_path
        self._num_samples = num_samples
        self._max_sample_len = max_sample_len
        self._input_texts = []
        self._target_texts = []
    ...
    def read_data(self):
        # This will adapt to whatever compression style is needed (supported).
        print("Reading text corpus ...")
        sys.stdout.flush()
        with tarfile.open(self._data_path, 'r') as tar:
            for tfile in tar.getmembers():
                if tfile.name[-2:] == 'en':
                    f = tar.extractfile(tfile)
                    input_lines = f.readlines()
                if tfile.name[-2:] == 'fr':
                    f = tar.extractfile(tfile)
                    target_lines = f.readlines()

        print("Organizing text corpus ...")
        sys.stdout.flush()
        i = 0
        while len(self._input_texts) < self._num_samples and i < len(input_lines) - 1:
            input_text = input_lines[i].strip()
            target_text = target_lines[i].strip()
            i += 1
            # If the line is too long, we will skip it. Long lines create large inefficiencies
            # when one-hot encoding all the inputs and outputs. This becomes especially bad when
            # the occurrence of long lines is very low in the dataset.
            if len(input_text) > self._max_sample_len or len(target_text) > self._max_sample_len:
                continue
    ...
    def get_num_encode_tokens(self):
        return self._data_stats['num_encoder_tokens']

    def get_num_decode_tokens(self):
