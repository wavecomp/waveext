...
class Train(object):
    ...
    '''
    def __init__(self, use_tensorboard=False, use_trace=False, arithmetic_type='tf'):
        # Set up all the placeholders
        self._use_tensorboard = use_tensorboard
        self._use_trace = use_trace
        self._arithmetic_type = arithmetic_type
    ...
    def train(self):
        ...
        tb_log = waveflow.TF_TBLogger(log_dir=train_dir,
                                      enable_tb=self._use_tensorboard,
                                      enable_trace=self._use_trace,
                                      unified_trace=True,
                                      arith_type=self._arithmetic_type)
        ...
        for step in range(FLAGS.train_steps):
            ...
            if step % FLAGS.report_freq == 0:
                ...
                if FLAGS.is_full_validation is True:
                    ...
                    tb_log._tb_writer.add_summary(vali_summ, step)
                    tb_log._tb_writer.flush()

                else:
                     val_res = tb_log.run_session(iter=step,
                                                  ops=[self.val_op, self.vali_top1_error, self.vali_loss],
                                                  feed={self.image_placeholder: train_batch_data,
                     ...
                     validation_error_value = val_res[1]
                     validation_loss_value = val_res[2]
            ...
            train_res = tb_log.run_session(iter=step,
                                          ops=[self.train_op, self.train_ema_op,
            ...
            train_loss_value = train_res[2]
            train_error_value = train_res[3]

            duration = time.time() - start_time


            if step % FLAGS.report_freq == 0:
                summary_str = tb_log.run_session(iter=step, ops=[],
                                                 feed={self.image_placeholder: train_batch_data,
                                                 self.label_placeholder: train_batch_labels,
                                                 self.vali_image_placeholder: validation_batch_data,
                                                 self.vali_label_placeholder: validation_batch_labels,
...
# Arguments definition is in hyper_parameters.py

maybe_download_and_extract()
# Initialize the Train object
train = Train(use_tensorboard=FLAGS.tensorboard, use_trace=FLAGS.trace, arithmetic_type=FLAGS.arith)
