...
def test_softmax():
    ...
    v_a = tf.Variable(tf.truncated_normal([100, 10], mean=0, stddev=5), name="a")
    v_da = tf.Variable(tf.truncated_normal([100, 10], mean=0, stddev=1), name="da")
    ...
    widgets = ["Softmax BP test: ", pb.Percentage(), ' ', pb.Bar(), ' ', pb.ETA()]
    ...
    for i in range(iters):
        ...
        with tf.Session('') as sess:
            ...
            z_op = waveflow.wavecomp_ops_module.wave_softmax_dfx(v_a)
            z_op2 = tf.nn.softmax(v_a)
