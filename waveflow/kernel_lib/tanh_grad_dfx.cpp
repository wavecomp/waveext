...
class WaveTanhGradDfxOp : public WaveDynFxPointOp {
public:
    typedef std::vector<DynFxPoint> DFXVector;

    explicit WaveTanhGradDfxOp(OpKernelConstruction* ctx)
    : WaveDynFxPointOp(ctx, {"bp_i0", "bp_i1", "bp_o0"}), m_mm_a_dfx(),
      m_mm_g_dfx(), m_mm_z_dfx()
    {
        if (m_show_banner) {
            printf("WaveTanhGradDfxOp() init\n");
            m_show_banner = false;
        }
    }
    ...
    void Compute(OpKernelContext* context) override
    {
        ...
        DynFxPoint dfx_one, dfx_tmp, p;
        dfx_one = 1.0f;
        dfx_tmp.set_fxbp(14, 16);
        p.set_fxbp(14 + g_bp.m_bp, 32);
        for (int i = 0; i < a_m.size(); i++) {
            dfx_tmp = m_mm_a_dfx[i] * m_mm_a_dfx[i];
            dfx_tmp = dfx_one - dfx_tmp;
            p = dfx_tmp * m_mm_g_dfx[i];
...
bool WaveTanhGradDfxOp::m_show_banner = true;

REGISTER_KERNEL_BUILDER(Name("WaveTanhGradDfx").Device(DEVICE_CPU), WaveTanhGradDfxOp);
