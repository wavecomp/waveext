 ...
 * */
class WaveQuantizeDfx : public WaveDynFxPointOp {
public:
    explicit WaveQuantizeDfx(OpKernelConstruction* ctx)
    : WaveDynFxPointOp(ctx, {"bp_i0", "bp_o0"})
    {
        if (m_show_banner) {
            printf("WaveQuantizeDfx() init\n");
            m_show_banner = false;
        }
    ...
    void Compute(OpKernelContext* context) override
    {
        ...
        auto a_m = a_input.flat<float>();
        fxbp a_bp = get_fxbp(true, 0);

        m_ma.resize(a_m.size());
        partial_in(a_bp, m_ma, a_m.data());
        ...
        // TODO: This function will have a configurable quantization method.
        partial_out(out_bp, output->flat<float>().data(), m_ma);
...
bool WaveQuantizeDfx::m_show_banner = true;

REGISTER_KERNEL_BUILDER(Name("WaveQuantizeDfx").Device(DEVICE_CPU), WaveQuantizeDfx);
