...
REGISTER_OP("WaveConv2DGradientWeight")
    .Input("input: float32")
...
class WaveConv2dGradientWeightOp : public OpKernel {
public:
    ...
    explicit WaveConv2dGradientWeightOp(OpKernelConstruction* ctx) : OpKernel(ctx) {
        ...
        */
        ...
        if (m_show_banner) {
            printf("WaveConv2dGradientWeightOp() init\n");
            m_show_banner = false;
        }
    }
    ...
    void Compute(OpKernelContext* context) override {
        ...
        OP_REQUIRES(context, (t_sizes.flat<int32>()(0) % 2 != 0),
            errors::InvalidArgument(
            "wave_conv2d_gradient_weight requires odd filter size"));
        ...
        gradient_conv2d_weight(t_gradient, t_input, *output);
    }
...
private:
    ...
    void gradient_conv2d_weight(const Tensor& t_grad, const Tensor& t_input, Tensor& t_out)
    {
        ...
        FPMatrix2d  mm_out(m_kdim*m_kdim*m_channel_in, m_channel_out);
        // Clear the output matrix first.
        fp_clear(mm_out, mm_out.rows(), mm_out.cols());
        
#ifdef OPENMP_OPTIMIZATION
        #pragma omp parallel for
#endif
        for(int i = 0; i < m_batch; ++i) {
            FPMatrix2d  thread_mm_in(m_out_h*m_out_w, m_kdim*m_kdim*m_channel_in);
            ...
            im2col(thread_mm_in, input_batch);

            partial_in(thread_mm_grad, out_batch);

            mm_tn(false, thread_mm_in, thread_mm_in.rows(), thread_mm_in.cols(),
        ...
        // Uses sum to preserve any outside data in m_conv_out.
        partial_out(m_conv_out.data(), mm_out);
    }
    
    float im2col_add_pixel(const float* im, int row, int col, int channel)
    {
        row -= m_pad;
        col -= m_pad;
...
bool WaveConv2dGradientWeightOp::m_show_banner = true;

REGISTER_KERNEL_BUILDER(Name("WaveConv2DGradientWeight").Device(DEVICE_CPU), WaveConv2dGradientWeightOp);
