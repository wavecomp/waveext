...
class WaveTanhIntOp : public WaveDynFxPointOp {
public:
    explicit WaveTanhIntOp(OpKernelConstruction* ctx)
    : WaveDynFxPointOp(ctx, {"bp_i0", "bp_o0"})
    {
        if (m_show_banner) {
            printf("WaveTanhIntOp() init\n");
            m_show_banner = false;
        }
    }
    ...
    void Compute(OpKernelContext* context) override
    {
        ...
        if (a_bp.m_bp > 15) {
            ...
            for (int i = 0; i < n_ext>>2; i++) {
                ...
                xk  = _mm_abs_epi32(xa);
                xk  = _mm_srai_epi32(xk, a_bp.m_bp - 15);
          ...
          for (int i = 0; i < n_ext>>2; i++) {
            ...
            xv1 = _mm_load_si128((__m128i*)z);
            xk  = _mm_abs_epi32(xa);
            xk  = _mm_slli_epi32(xk, 15 - a_bp.m_bp);
            xk  = _mm_and_si128(xk, xmask);
            xt  = _mm_slli_epi32(xk, 16);
            xk  = _mm_or_si128(xk, xt);
            xv2 = _mm_madd_epi16(xv1, xk);
...
#if !defined(USE_VECTOR_INSTRUCTIONS)
    int32_t tanh(int32_t idx, int32_t r)
    {
        int32_t v1, v2;

        if (idx > TANH_LUT_SIZE - 2)
            return 16384;
        else {
            v1 = tanh_lut[idx];
            v2 = tanh_lut[idx+1];
...
bool WaveTanhIntOp::m_show_banner = true;

REGISTER_KERNEL_BUILDER(Name("WaveTanhInt").Device(DEVICE_CPU), WaveTanhIntOp);
