...
REGISTER_OP("WaveSigmoidGradInt")
    .Input("a: float")
    .Input("g: float")
    .Output("z: float")
    .Attr("bp_i0: string = ''")
...
class WaveSigmoidGradIntOp : public WaveDynFxPointOp {
public:
    typedef std::vector<DynFxPoint> DFXVector;

    explicit WaveSigmoidGradIntOp(OpKernelConstruction* ctx)
    : WaveDynFxPointOp(ctx, {"bp_i0", "bp_i1", "bp_o0"})
    {
        if (m_show_banner) {
            printf("WaveSigmoidGradIntOp() init\n");
            m_show_banner = false;
        }
    }
    ...
    void Compute(OpKernelContext* context) override
    {
...
#if defined(USE_VECTOR_INSTRUCTIONS)
        __m128i xmask1 = _mm_set1_epi32(0xfff);
        __m128i xmask2 = _mm_set1_epi32(0x3fff);
        __m128i xround1 = _mm_set1_epi32(0x800);
        __m128i xround2 = _mm_set1_epi32(0x2000);
        ...
        for (int i = 0; i < n_ext>>2; i++) {
            ...
            xt = _mm_sub_epi32(xbias, xv);
            xt = _mm_mullo_epi32(xt, xv);
            xv = _mm_and_si128(xt, xmask1);
            xt = _mm_add_epi32(xt, xround1);
            xt = _mm_srai_epi32(xt, 12);
            xv = _mm_cmpeq_epi32(xv, xround1);
            xv = _mm_sub_epi32(xv, xone);
            xt = _mm_and_si128(xt, xv);

            xv = _mm_load_si128((__m128i*)g);
            xt = _mm_mullo_epi32(xt, xv);
            xv = _mm_and_si128(xt, xmask2);
            xt = _mm_add_epi32(xt, xround2);
            xt = _mm_srai_epi32(xt, 14);
            xv = _mm_cmpeq_epi32(xv, xround2);
            xv = _mm_sub_epi32(xv, xone);
            xt = _mm_and_si128(xt, xv);
...
bool WaveSigmoidGradIntOp::m_show_banner = true;

REGISTER_KERNEL_BUILDER(Name("WaveSigmoidGradInt").Device(DEVICE_CPU), WaveSigmoidGradIntOp);
